  ...
  inventories.forEach((inv) => {
    ...
    Object.keys(inventory).forEach((collection) => {
      ...
      Object.keys(items).forEach((asset) => {
        ...
        items[asset].img.type =
          items[asset].img.src.endsWith('.mp4') || items[asset].img.src.endsWith('.mov') ? 'video' : 'img'
        items[asset].thumbnail = { src: thumbnail || animation || img || '' }
        items[asset].thumbnail.type =
          items[asset].thumbnail.src.endsWith('.mp4') || items[asset].thumbnail.src.endsWith('.mov')
            ? 'video'
            : 'img'
      })
    })
