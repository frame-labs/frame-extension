...
class RightPanel extends React.Component {
    ...
    return (
      <PopRight key='hover:user'>
        <PopUser>
          ...
          <ProfileMatch>{`@${user.twitter}  -  ${user.name}`}</ProfileMatch>
          {user.verified.name ? (
            <>
              <PopUserVerified>
                <PopUserLabel>{'ens name verified'}</PopUserLabel>
              </PopUserVerified>
            </>
          ) : (
            <>
              <PopUserUnverified>
                <PopUserLabel>{'ens name unverified'}</PopUserLabel>
              </PopUserUnverified>
            </>
          )}
          <ProfileImage>
  ...
  renderAsset(user, collection, asset) {
    ...
    const tokenId = assetData?.tokenId
      ? '#' +
        (assetData.tokenId.length > 9
          ? assetData.tokenId.substr(0, 3) + '...' + assetData.tokenId.substr(-3)
          : assetData.tokenId)
      : '?'
    ...
    return (
      <PopRight key={assetData.name}>
        <AssetSummaryWrap>
          ...
          <AssetSummaryName>{assetData.name}</AssetSummaryName>
          <AssetSummaryCollection>
            <div>
              {collectionName === 'ENS: Ethereum Name Service' ? 'Ethereum Name Service' : collectionName}
            </div>
            <div>{tokenId}</div>
          </AssetSummaryCollection>
          <AssetLink
  ...
  renderCollection(user, collection) {
    ...
    return (
      <PopRight key={collectionData.meta.name}>
        <CollectionSummaryTitle>
          {collectionData.meta.name === 'ENS: Ethereum Name Service'
        ...
        <CollectionItemCount>
          <CollectionItemCountNumber>{count}</CollectionItemCountNumber>
          <CollectionItemCountLabel>{count === 1 ? 'item' : 'items'}</CollectionItemCountLabel>
        </CollectionItemCount>
      </PopRight>
    )
  }
